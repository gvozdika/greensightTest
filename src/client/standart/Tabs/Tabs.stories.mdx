...
import Tabs from '.';
import Accordion from '@standart/Accordion';

<Meta title="Standart / Tabs" component={Tabs} decorators={[centeredDecorator]} />
...
☝🏿 При использовании табов на SSR нужно производить сброс счётчика для матчинга id между сервером и клиентом. Для этого в `server/render.js` производится вызов **resetIdCounter** (см. [документацию](https://github.com/reactjs/react-tabs#resetidcounter-void)). В целом, если вы не меняли серверную часть, то ничего делать не нужно, просто FYI.
